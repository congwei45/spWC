...
class Trainer:
    ...
    def before(self, cur_epoch, train_loader):
        ...
        elif self.pseudo_labeling.split("_")[0] == "proto" and self.step > 0:
    ...
    def train(self, cur_epoch, optim, train_loader, scheduler=None, print_int=10, logger=None):
        ...
        old_classes_difficulty = torch.ones(256)
        reg = torch.tensor(0.)
        ...
        # model.train()
        if self.step > 0:
            with open('results/2023-06-01_voc_15-5s_RCIL_overlap.csv', 'r') as f:
                for line_index, line in enumerate(f):
                    split = line.split(',')
                    old_classes_accuracy = line.split(',')[1:self.old_classes+1]
            #print('old_classes_accuracy', old_classes_accuracy)
            old_classes_accuracy = [float(i) for i in split[1:self.old_classes+1] if i not in ('x', 'X')]
            old_classes_accuracy = [torch.tensor(i) for i in old_classes_accuracy]
            for i in range(len(old_classes_accuracy)):
                old_classes_difficulty[i] = -old_classes_accuracy[i]*torch.log(1-old_classes_accuracy[i])
            old_classes_difficulty = old_classes_difficulty.to(device)
            #print('old_classes_difficulty', old_classes_difficulty)
        ...
        for cur_step, (images, labels) in enumerate(train_loader):
            ...
            if self.pseudo_soft is not None:
                ...
                if self.ce_on_pseudo and self.step > 0:
                    ...
                    if self.step > 0:
                    #print('labels', )
                        count = (labels < self.old_classes).sum()
                        mask = labels < self.old_classes
                        reg = (1/3 * torch.sqrt(torch.sum(old_classes_difficulty[labels][mask] ** 3))-torch.sum(old_classes_difficulty[labels][mask]))/count
                        #print('reg', reg)

                        loss = old_classes_difficulty[labels] * criterion(outputs, labels)  # B x H x W
                    else:
            ...
            loss_tot = loss + lkd + lde + l_icarl + pod_loss + loss_entmin + reg
        ...
        with torch.no_grad():
            for i, (images, labels) in enumerate(loader):
                ...
                for kk in range(images.shape[0]):
                    torchvision.utils.save_image(images[kk],
                                                 f'./results-ours/{self.method}/{i}_{kk}_{self.local_rank}_{self.step}_image.png',
                                                 normalize=True)
                ...
                for kk in range(outputs.shape[0]):
                    ...
                    Image.fromarray(pred_mask).save(
                        f'./results-ours/{self.method}/{i}_{kk}_{self.local_rank}_{self.step}.png')
                    Image.fromarray(
                        Label2Color(color_map('voc'))(labels[kk].detach().cpu().numpy()).astype(np.uint8)).save(
                        f'./results-ours/{self.method}/{i}_{kk}_{self.local_rank}_{self.step}_gt.png')
